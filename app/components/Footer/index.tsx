...
const documentLinks: FooterItem[] = [
  {
    name: "Политика конфиденциальности",
    url: "/politic_conf.pdf",
    external: true,
  },
  {
...
export function Footer() {
  ...
  // /map is a full-screen map (100dvh) — a footer after it would
  // create page overflow and push the map's bottom off-screen.
  if (
    ["/login", "/register", "/forgot", "/chat", "/map"].includes(
      pathname || ""
    )
  ) {
    return null;
  ...
  return (
    <footer
      className="w-full font-[family-name:var(--font-stetica-regular)] lg:mt-[35px]"
      style={{
    ...
    >
      <div
        className="max-w-[1300px] mx-auto px-4 sm:px-6 lg:px-7 py-3 lg:py-6 footer-inner"
      >
        {/* Main navigation — desktop only */}
        <div className="hidden lg:flex flex-col sm:flex-row items-center justify-between gap-4 sm:gap-6 w-full pb-4">
          ...
          >
            <ul className="flex flex-wrap justify-center gap-x-4 gap-y-2 sm:gap-x-6 lg:gap-x-10">
              {navigationLinks.map((item) => (
                <li key={item.name}>
                  {item.url && (
                    <Link
                      href={item.url}
                      className="text-sm sm:text-base leading-6 focus:outline-none transition-all hover:opacity-80"
                  ...
                  {!item.url && (
                    <SupportTooltip placement="top">
                      <p
                        className="cursor-pointer text-sm sm:text-base leading-6 focus:outline-none transition-all hover:opacity-80"
                        style={{ color: "var(--text-secondary)" }}
                      >
                        {item.name}
                      </p>
        ...
        {/* Documents + copyright */}
        <div className="lg:pt-4 lg:border-t lg:border-[var(--border-color)]">
          <nav aria-label="Документы" className="flex justify-center">
          ...
          >
            Dream House {new Date().getFullYear()}
          </p>
        </div>
      </div>
      <style jsx>{`
        @media (max-width: 1023px) {
          .footer-inner {
            padding-bottom: calc(12px + 88px + env(safe-area-inset-bottom));
          }
        }
      `}</style>
    </footer>
